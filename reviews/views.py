...
def add_review(request, product_id):
    """
    Allows users to add a review. Also checks if the user has purchased
    the product to show a purchase verification message.
    """
    if not request.user.is_authenticated:
        messages.error(request,
                       'Sorry, you need to be logged in to add a review.')
    ...
    has_bought = OrderLineItem.objects.filter(
    ...
    context = {
        'review_form': review_form,
        'product': product,
        'has_bought': has_bought,  # ✅ Pass to template
    }
